...
export default function Home() {
  return (
    <main className="min-h-screen bg-[#070907] text-[#f5f2e9]">
      <header className="sticky top-0 z-30 border-b border-white/10 bg-[#070907]/90 backdrop-blur">
        <nav className="mx-auto flex max-w-6xl items-center justify-between px-5 py-4">
          ...
          <a
            ...
            className="rounded-full bg-[#c8ef6a] px-4 py-2 text-sm font-semibold text-[#071007] transition hover:bg-white"
          >
            Hire me
          </a>
        </nav>
      ...
      <section id="home" className="mx-auto grid max-w-6xl gap-12 px-5 py-16 md:grid-cols-[1.05fr_0.95fr] md:py-24">
        <div className="flex flex-col justify-center">
          <p className="mb-5 text-sm font-bold uppercase tracking-[0.28em] text-[#c8ef6a]">
        ...
        <div className="relative min-h-[430px] overflow-hidden rounded-[8px] border border-white/10 bg-[#10150f] p-5 text-white shadow-2xl shadow-black/35">
      ...
      <section id="about" className="border-y border-white/10 bg-[#0d120d]">
        <div className="mx-auto grid max-w-6xl gap-10 px-5 py-16 md:grid-cols-[0.85fr_1.15fr]">
          <div>
            <p className="text-sm font-bold uppercase tracking-[0.24em] text-[#c8ef6a]">About</p>
      ...
      <section id="skills" className="mx-auto max-w-6xl px-5 py-16">
        <div className="mb-9 flex flex-col justify-between gap-4 md:flex-row md:items-end">
          <div>
            <p className="text-sm font-bold uppercase tracking-[0.24em] text-[#c8ef6a]">Toolkit</p>
            <h2 className="mt-4 text-3xl font-black sm:text-4xl">Skills built for reporting, analysis, and automation.</h2>
          </div>
        </div>
        <div className="grid gap-4 md:grid-cols-4">
          {skills.map((group) => (
            <article key={group.title} className="rounded-[8px] border border-white/10 bg-[#111810] p-5">
              <h3 className="text-lg font-black">{group.title}</h3>
              <div className="mt-5 flex flex-wrap gap-2">
                {group.items.map((item) => (
                  <span key={item} className="rounded-full bg-[#1f2b1d] px-3 py-1 text-sm font-semibold text-[#d7f98b]">
                    {item}
                  </span>
                ))}
              </div>
      ...
      <section id="experience" className="bg-[#111810] text-white">
      ...
      <section id="projects" className="mx-auto max-w-6xl px-5 py-16">
        <p className="text-sm font-bold uppercase tracking-[0.24em] text-[#c8ef6a]">Projects</p>
        ...
        <div className="mt-9 grid gap-5 lg:grid-cols-3">
          {projects.map((project) => (
            <article key={project.title} className="flex flex-col rounded-[8px] border border-white/10 bg-[#111810] p-6">
              <p className="text-sm font-bold uppercase tracking-[0.18em] text-[#c8ef6a]">{project.stack}</p>
              <h3 className="mt-4 text-2xl font-black leading-tight">{project.title}</h3>
              <p className="mt-4 text-base leading-7 text-white/66">{project.summary}</p>
      ...
      <section className="border-y border-white/10 bg-[#0d120d]">
        <div className="mx-auto grid max-w-6xl gap-8 px-5 py-16 md:grid-cols-2">
          <div>
            <p className="text-sm font-bold uppercase tracking-[0.24em] text-[#c8ef6a]">Education</p>
            ...
            <p className="mt-2 font-semibold text-white/66">2020 - 2024 | CGPA: 7.18</p>
          </div>
          <div className="rounded-[8px] border border-white/10 bg-[#111810] p-6">
            <p className="text-sm font-bold uppercase tracking-[0.2em] text-[#c8ef6a]">Current focus</p>
